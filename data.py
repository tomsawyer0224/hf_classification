...
class BaseDataset:
    ...
    def generate_datasets(self):
        ...
        if hasattr(self.raw_dataset, "keys"): # if DatasetDict
            ...
            class_names = self.raw_dataset["train"].features[self.label_column_name].names
            ...
            if not has_validation and not has_test:
                train_dataset, validation_dataset, test_dataset = self.train_validation_test_split(
                    dataset = raw_train_dataset
                )
            elif not has_validation and has_test:
                train_validation = raw_train_dataset.train_test_split(test_size = 0.1, seed = 42)
                train_dataset = train_validation["train"]
                validation_dataset = train_validation["test"]
...
class ImageClassificationDataset(BaseDataset):
    def __init__(
        ...
        self.image_processor = transformers.AutoImageProcessor.from_pretrained(checkpoint)
        #self.image_size = image_size
        #self.batch_size = batch_size
        (self.train_dataset, 
        self.validation_dataset, 
        self.test_dataset, 
        self.class_names) = self.generate_datasets()
    ...
    def prepare_datasets(self):
        ...
        def _train_transform(examples):
            ...
            examples['pixel_values'] = [
                aug_transformation(pil_img.convert("RGB")) for pil_img in pil_images_list
            ]
            return examples
        def _val_transform(examples):
            pil_images_list = examples['image']
            examples['pixel_values'] = [
                normal_transformation(pil_img.convert("RGB")) for pil_img in pil_images_list
            ]
            return examples

        train_dataset = train_dataset.map(
            function = _train_transform,
            batched = True,
            batch_size = 128,
            num_proc = 2
        )
        validation_dataset = validation_dataset.map(
            function = _val_transform,
            batched = True,
            batch_size = 128,
            num_proc = 2
        )
